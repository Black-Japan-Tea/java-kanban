...
public class SubtasksHandlerTest {
    ...
    public void shouldAddSubtask() throws IOException, InterruptedException {
        ...
        Subtask subtask = new Subtask("Subtask 1", "Testing Subtasks 1", Status.DONE, 1,
        ...
        Collection<Subtask> tasksFromManager = taskManager.getSubtasks();
        assertNotNull(tasksFromManager, "Задачи не возвращаются");
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        assertEquals("Subtask 1", tasksFromManager.iterator().next().getName(),
                "Некорректное имя задачи");
        assertEquals(epic.getSubtasks().getFirst(), tasksFromManager.iterator().next().getId(),
    ...
    public void shouldUpdateSubtask() throws IOException, InterruptedException {
        ...
        assertNotNull(tasksFromManager, "Задачи не возвращаются");
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        assertEquals("Subtask Update", tasksFromManager.iterator().next().getName(),
                "Некорректное имя задачи");
    }
    ...
    public void shouldDeleteSubtask() throws IOException, InterruptedException {
        ...
        Collection<Subtask> tasksFromManager = taskManager.getSubtasks();
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        assertEquals("Test Save", tasksFromManager.stream().iterator().next().getName(),
                "Некорректное имя задачи");
        assertEquals(epic.getSubtasks().getFirst(), tasksFromManager.iterator().next().getId(),
                "Неверный id в хранилище подзадач эпика");
    }
